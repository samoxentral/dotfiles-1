...
local git = require 'user.git.utils.git'
...
local function on_blame_done(lines)
  ...
  vim.api.nvim_buf_set_keymap(0, 'n', '<CR>', "<CMD>lua require('user.git.blame').blame_commit()<CR>", options)
  vim.api.nvim_command "autocmd BufWinLeave <buffer> lua require('user.git.blame').blame_quit()"
...
local function on_blame_commit_done(commit_hash, lines)
  ...
  vim.api.nvim_command "autocmd BufLeave <buffer> lua require('user.git.blame').blame_commit_quit()"
